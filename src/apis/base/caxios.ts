...
import { Toast, showLoadingToast, showFailToast } from 'vant';
    ...
    this.service.interceptors.response.use(
      response => {
        ...
        if (String(responseCode) !== '200' && String(responseCode) !== '200010000' && String(responseCode) !== '0') {
          ...
          showFailToast(response.data.message || response.data.msg || '系统异常');
        ...
        if (error.response) {
          ...
          switch (errorCode) {
            ...
            case 500:
              ...
              break;
            case 501:
              showFailToast('请求还没有被实现');
              break;
            case 502:
              showFailToast('网关错误');
              break;
            case 503:
              showFailToast('服务暂时不可用');
              break;
            case 505:
              showFailToast('请求的 HTTP 版本不支持');
              break;
            default:
              break;
          }
